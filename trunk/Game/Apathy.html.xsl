...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        ...
        <div class="toc" id="Table-of-Contents">
          ...
          <ol class="toc">
            <xsl:for-each select="//part">
              <li>
                <a href="#part-{translate(./title,' ','')}">
                  <xsl:value-of select="title" />
                </a>
                <ol class="toc" >
                  <xsl:for-each select="chapter">
                    <li>
                      <a href="#chapter-{translate(./title,' ','')}">
                        <xsl:value-of select="title" />
                      </a>
                      <ol class="toc" >
                        <xsl:for-each select="section">
                          <li>
                            <a href="#section-{translate(../../title,' ','')}-{translate(../title,' ','')}-{translate(./title,' ','')}">
                              <xsl:value-of select="title" />
                            </a>
                            <ol class="toc" >
                              <xsl:for-each select="section">
        ...
        <div class="examples" id="List-of-Examples">
          ...
          <ol class="examples">
            <xsl:for-each select="//example">
              <li><a href="#example-{translate(./title,' ','')}">
                <xsl:value-of select="title" />
              </a></li>
            </xsl:for-each>
          </ol>
        ...
        <div class="figures" id="List-of-Figures">
          ...
          <ol class="figures">
            <xsl:for-each select="//figure">
              <li>
                <a href="#figure-{translate(./caption,' ','')}">
  ...
  <xsl:template match="book">
    <xsl:for-each select="part">
      <div class="part" id="part-{translate(./title,' ','')}">
        <h1><xsl:value-of select="title" /></h1>
        <xsl:for-each select="chapter">
          <div class="chapter" id="chapter-{translate(./title,' ','')}">
            <!--<h1><xsl:value-of select="title" /></h1>-->
            <xsl:apply-templates />
          </div>
        </xsl:for-each>
  ...
  <xsl:template match="section">
    <div class="section" id="section-{translate(../../title,' ','')}-{translate(../title,' ','')}-{translate(./title,' ','')}" >
      <!--<h1><xsl:value-of select="title" /></h1>-->
      <xsl:apply-templates />
    </div>
  </xsl:template>
  ...
  <xsl:template match="example">
    <div class="example" id="example-{translate(./title,' ','')}">
      <h1>
  ...
  <xsl:template match="figure">
    <div class="figure" id="figure-{translate(./caption,' ','')}">
