...
def setUniqueID(apathy,id_init=[0]):
  for child in apathy.childNodes:
    if child.nodeType == child.ELEMENT_NODE:
      if child.attributes.has_key("unique-id"):
        del child.attributes["unique-id"];
      if not child.attributes.has_key("id"):
        id_init[0] += 1
        child.attributes["id"] = ""
      child.setAttribute("id",("G%7s"%str(id_init[0])).replace(" ","0"))
      setUniqueID(child)
